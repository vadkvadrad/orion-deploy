...
- Ansible коллекция `community.docker` (для модулей работы с Docker: логин в registry, управление образами и docker-compose):
  ...
  ```
- Docker и Docker Buildx (только если `build_on_remote: false`, сборка выполняется локально)
  
  **Примечание:** По умолчанию сборка Docker-образов выполняется локально (`build_on_remote: false`). Если установить `build_on_remote: true`, сборка будет выполняться на удаленных серверах, и Docker/Buildx на локальной машине не потребуются.
...
build_on_remote: false                                  # Сборка на удаленном сервере (true) или локально (false, по умолчанию)
...
2. **Build and push Docker image** - сборка образа из Git-репозитория и публикация в registry (локально или на удаленном сервере, в зависимости от `build_on_remote`)
3. **Deploy application** - деплой приложения через docker-compose

**Важно:** По умолчанию сборка выполняется локально (`build_on_remote: false`). Если установить `build_on_remote: true`, сборка будет выполняться на удаленных серверах, и убедитесь, что серверы имеют доступ к Git-репозиторию и Docker registry.
...
### Пример 4: Сборка на удаленном сервере

Если вы хотите выполнять сборку на удаленном сервере вместо локальной машины:
...
build_on_remote: true  # Сборка будет выполняться на удаленном сервере
...
**Примечание:** При `build_on_remote: true` убедитесь, что удаленный сервер имеет доступ к Git-репозиторию и Docker registry.
...
| `build_on_remote` | ❌ | `false` | Сборка на удаленном сервере (true) или локально (false) |
...
- Если `build_on_remote: false` (по умолчанию): убедитесь, что Docker и buildx установлены локально
- Если `build_on_remote: true`: убедитесь, что Docker установлен на удаленном сервере (роль `docker` установит его автоматически)
- Проверьте, что в репозитории есть `Dockerfile`
- Проверьте доступ к Git-репозиторию (с локальной машины или с удаленного сервера, в зависимости от `build_on_remote`)
